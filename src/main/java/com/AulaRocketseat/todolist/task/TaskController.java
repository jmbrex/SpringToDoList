...
import com.AulaRocketseat.todolist.Util.Util;
...
public class TaskController {
    ...
    @PostMapping("/")
    public ResponseEntity creatre(@RequestBody TaskModel taskModel, HttpServletRequest request){
        ...
        //salvando a task evniada e retornando a taks criada
        taskModel.setIdUser((Long) request.getAttribute("idUser"));
        var task = this.taskRepository.save(taskModel);
        return ResponseEntity.status(HttpStatus.CREATED).body(task);
    }
    
    @GetMapping("/")
    public ResponseEntity lits(HttpServletRequest request){
        var tasks = this.taskRepository.findByidUser((Long) request.getAttribute("idUser"));
        return ResponseEntity.status(HttpStatus.OK).body(tasks);
    }
    
    @PutMapping("/{id}")
    public ResponseEntity updade(@RequestBody TaskModel taskModel, HttpServletRequest request, @PathVariable Long id){
